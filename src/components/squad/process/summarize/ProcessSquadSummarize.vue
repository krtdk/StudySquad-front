  ...
  <v-container class="container">
    ...
    <div class="container_body">
      <div
        ...
        <div class="create_board_button">
          <v-btn
            class="button_style"
            @click="handleAllowedBoard">
            게시글 생성
          </v-btn>
        </div>
      </div>
      <div
          class="container_body_fail"
          v-else>
        <div class="container_body_fail_description">
          작성한 게시물이 존재하지 않습니다.
        </div>
        <div class="create_board_button">
          <v-btn
...
export default {
  ...
  methods: {
    handleAllowedBoard() {
      ...
      BoardService.isBoardAllowed(squadId)
          .then(response => {
            if (response.data) {
              this.$router.push(`/squad/${this.squadId}/summarize/create`);
            }
          })
          .catch(error => {
            alert(error.response.data.message);
...
.container_body_fail_description {
  display: flex;
  flex-direction: row;
  justify-content: center;
  align-items: center;
